...
function generateTableColab(tableColab, userValues, min, max, scale=5) {
  ...
  //////////////////////////////////////////////////////////////////////////
  function makeDate(d){
    return `${d.toLocaleString('en-us', {weekday: 'long' })}    ${d.getHours()}:${d.getMinutes()}`
  }
  ...
  for (let sec of timeframe) {
    ...
    if( counter === 0) {
      counter = 60
      let cDate = new Date(sec * 60 * 1000)
      let head = row.insertCell()
      console.log(cDate)
      head.appendChild(document.createTextNode(makeDate(cDate)))
      head.colSpan = 60
      head.className = "top"
    }
    lastsec = sec
  }
  let cDate = new Date(lastsec * 60 * 1000)
  let head = row.insertCell()
  console.log(cDate)
  head.appendChild(document.createTextNode(makeDate(cDate)))
  head.colSpan = 60
  ...
  for (const [key, value] of Object.entries(userValues)) {
    ...
    for (let sec of timeframe) {
      console.log(new Date(sec*60*1000))
